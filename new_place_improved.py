...
features_dem = open('new_place_features_dem_' + sys.argv[1], 'w')
features_rep = open('new_place_features_rep_' + sys.argv[1], 'w')
...
for filename in os.listdir('data_senti/' + sys.argv[1]):
    ...
    if fields[1] in democrats:
        ...
        for i in range(1, 15):
            ...
            if(len(this_features2)==0):
                this_features2=(person_dates[fields[1]][from_date])
            else:
                for indexx, it in enumerate(person_dates[fields[1]][from_date]):
                    this_features2[indexx] += it
        this_features+=this_features2
